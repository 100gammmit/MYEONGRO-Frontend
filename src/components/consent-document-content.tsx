...
export function ConsentDocumentContent({
  ...
  if (documentType === "saju-input") {
    return (
      <div className="consent-document-content">
        ...
        <p>명로 서버에서 사주 명식과 흐름을 계산하고, 입력·계산 기준과 생성된 사주 리딩을 내 기록에 저장합니다. 원본 출생정보는 OpenAI API에 전송하지 않고, 서버에서 계산한 명식 정보만 질문과 함께 전송합니다.</p>
  ...
  return (
    <div className="consent-document-content">
      ...
      <p>소셜 로그인 계정 식별, 연령 이용자격 확인, AI 리딩 생성, 리딩 기록 저장·조회·삭제, 서비스 보안과 장애 대응을 위해 OAuth 프로필 정보, 만 19세 이상 확인 여부와 정책 버전·확인 시각, 이용자가 작성한 질문·선택지, 사주 리딩의 출생정보와 서비스 이용 기록을 처리합니다. 연령 확인을 위해 생년월일이나 신분증 정보는 수집하지 않습니다.</p>
