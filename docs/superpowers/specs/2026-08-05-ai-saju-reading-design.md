...
- 양력 생년월일
- 출생 시각 정확도: `정확히 알아요`, `대략적으로 알아요`, `시간을 몰라요`
- 정확하거나 대략적으로 아는 경우의 시각
- 출생 시·도
- 출생 시·군·구
...
`대략적으로 알아요`를 선택하면 입력한 시각을 중심으로 서버가 `±60분` 범위를 만든다는 설명을 표시한다. 오전·오후 정도만 아는 사용자는 거짓 정밀도를 만들지 않도록 `시간을 몰라요`를 선택하게 안내한다.
...
- 정확한 시각은 입력한 분 단위 시각을 사용한다.
- 대략적인 시각은 기억하는 시각의 `±60분` 범위에서 가능한 명식을 비교한다.
- 시간 미상은 시주를 임의 생성하지 않고 `미상`으로 표시한다.
- 시간 범위 또는 가능한 출생 시각 전체에서 동일한 계산 사실만 확정 근거로 사용한다.
- 가능한 시각에 따라 시주나 현재 대운이 달라지면 해당 항목을 제외하거나 두 가능성을 제한사항으로 표시한다.
...
- 00:00 일자 경계
- 자시 범위 23:00-00:59
- 대략적 출생 시각의 불확실성 범위 `±60분`
- 시간 범위 안의 시주·일주·절입·대운 경계를 모두 평가하는 후보 계산
...
대략적 시각은 정규화 payload에 서버가 계산한 범위를 함께 저장한다.
...
```json
{
  ...
  "birthTime": "14:00",
  "birthTimeWindow": {
    "start": "1992-08-17T13:00:00+09:00",
    "end": "1992-08-17T15:00:00+09:00"
  }
}
```

범위가 날짜를 넘으면 실제 이전·다음 날짜를 보존한다. 계산기는 범위의 시작과 끝만 비교하지 않고 범위 안의 시주, 일주, 절입 경계마다 후보를 만든다. 모든 후보에서 동일한 값만 trusted calculation의 확정 근거로 제공한다.
...
- `birthTimePrecision`은 `exact`, `approximate`, `unknown` 중 하나다.
- `exact`와 `approximate`면 `birthTime`이 필요하다.
- `unknown`이면 `birthTime`은 없어야 한다.
- `approximate`의 불확실성 범위는 클라이언트가 제출하지 않고 서버 규칙 `±60분`으로 결정한다.
...
- 대략적 시각의 후보 사이에서 달라지는 시주·대운을 하나로 확정하지 않음
...
- 대략적 시각, 시각 미상, 절기 경계, 시주 경계, 도시 대표 좌표 위험은 오류가 아니라 versioned limitation code로 저장한다.
...
- 시간 미상
- 대략적 시각 범위 안에서 시주가 하나로 유지되는 사례
- 대략적 시각 범위 안에서 시주·일주 또는 대운이 갈리는 사례
...
- 대략적 시각 후보에서 공통되지 않은 근거의 AI 입력 제외
...
- 시·도 변경 시 하위 선택 초기화
- 단일 하위 지역 자동 선택
- `정확히 알아요` 선택 시 분 단위 time input
- `대략적으로 알아요` 선택 시 기억하는 시각과 `±60분` 안내
- `시간을 몰라요` 선택 시 time input 비활성
- 대운 계산 기준 목적 안내
